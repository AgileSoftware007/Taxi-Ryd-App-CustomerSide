...
class UpdateVehicleDocumentVC: UIViewController {
    ...
    @IBAction func saveBtnAction(_ sender: Any) {
        if validation(){
            if Reachability.isConnectedToNetwork(){
                ...
                self.callApiUploadDrivingImageFirst(apiName: API_NAME.DRIVING_LICENCE_UPLOAD_FRONT)
            }else {
...
extension UpdateVehicleDocumentVC: UIImagePickerControllerDelegate,UINavigationControllerDelegate {
            ...
            case .success(_):
                do {
                    ...
                    if loginResponse.status == true {
                        let vc = UploadedDocumnet.getVC(.home)
